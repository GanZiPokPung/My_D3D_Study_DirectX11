...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	// 모델 객체 초기화
	if (!m_Model->Initialize(m_Direct3D->GetDevice(), L"../Dx11Demo_27/data/seafloor.dds", "../Dx11Demo_27/data/triangle.txt"))
	{
...
bool GraphicsClass::RenderToTexture()
{
	...
	XMMATRIX worldMatrix, reflectionViewMatrix, projectionMatrix, viewMatrix;
	
	// Set the render target to be the render to texture.
	// 렌더타겟뷰을 백버퍼가 아닌 텍스쳐로 설정
	m_RenderTexture->SetRenderTarget(m_Direct3D->GetDeviceContext(), m_Direct3D->GetDepthStencilView());

	// Clear the render to texture.
	// 텍스쳐를 지움 (BeginScene 기능과 비슷)
...
bool GraphicsClass::RenderScene()
{
	...
	m_Direct3D->GetWorldMatrix(worldMatrix);
	worldMatrix = XMMatrixRotationX(3.141592f / 2);
	worldMatrix *= XMMatrixTranslation(0.f, 0.f, 5.0f); 
