...
bool TextClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, HWND hwnd, int screenWidth, int screenHeight, XMMATRIX baseViewMatrix)
{
	...
	// 문장 정점 버퍼를 새 문자열 정보로 업데이트합니다.
	result = UpdateSentence(m_sentence3, "", 400, 300, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetKeyCharacter(char* character, ID3D11DeviceContext *deviceContext)
{
	char keyString[16] = { 0, };
	strcpy_s(keyString, "KeyInput: ");
	strcat_s(keyString, character);

	if (!UpdateSentence(m_sentence3, keyString, 20, 60, 1.f, 1.f, 1.f, deviceContext))
	{
		return false;
	}
